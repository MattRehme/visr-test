...
out vec2  v_uv;
out float v_merc_x;      // original merc x (no wrap offset) for bounds clipping
...
void main() {
  // Discard fragments from world-wrap copies that fall outside [-180,180]
  if (v_merc_x < 0.0 || v_merc_x > 1.0) { fragColor = vec4(0.0); return; }
...
function makeScalarLayer(id, grid, colorStops, vmin0, vmax0, toggleId, opacityId) {
  ...
  return {
    ...
    render(gl, args) {
      ...
      // Three world-copies for seamless Mercator panning; globe uses only center.
...
function buildPopupHTML(lat, lng) {
  ...
  if (windOn) {
    ...
    extra += `<div class="popup-zoom">${gridLabel(WIND_GRID, 'WIND').toUpperCase()} ${Math.max(0, w).toFixed(4)}</div>`;
  }
  return `
...
map.on('mousemove', (e) => {
  ...
  document.getElementById('lng-val').textContent = lngWrapped.toFixed(4);
  ...
  if (windOn) {
    const w = gridLookup(WIND_GRID, lat, lng);
    document.getElementById('wind-val').textContent = isNaN(w) ? '—' : Math.max(0, w).toFixed(4);
